...
bool core::get_block_template(Block& b, const AccountPublicAddress& adr, difficulty_type& diffic, uint32_t& height, const BinaryArray& ex_nonce) {
...

  {
    ...
    //@jagerman's patch 
    uint64_t check_window = m_blockchain.getForkVersion() < 2 ? parameters::BLOCKCHAIN_TIMESTAMP_CHECK_WINDOW : parameters::BLOCKCHAIN_TIMESTAMP_CHECK_WINDOW_V2;
    if(m_blockchain.getCurrentBlockchainHeight() >= check_window){
      ...
      {
        timestamps.push_back(m_blockchain.getBlockTimestamp(static_cast<uint32_t>(offset)));
      }
      
      double median;
      size_t ts_size = timestamps.size();
      if (ts_size % 2 == 0){
        median = (timestamps[ts_size / 2 - 1] + timestamps[ts_size / 2]) / 2.0;
      }
      else{
        median = static_cast<double>(timestamps[ts_size / 2]);
      }
    ...
    median_size = static_cast<size_t>(m_blockchain.getCurrentCumulativeBlocksizeLimit() / 2);
  ...
  //10 attempts to get valid base transaction
  ...
  for(size_t i=0;i<10;i++){
	  ...
	  if ((block_reward + fee < checkReward) || (checkReward < block_reward)) {
		logger(ERROR, BRIGHT_RED) << "Coinbase transaction would fit reward amount, attempting to construct a new one -> " << m_currency.formatAmount(checkReward) <<
  ...
  for (size_t try_count = 0; try_count != 10; ++try_count) {
    ...
    if (!r) { 
      logger(ERROR, BRIGHT_RED) << "Failed to construct miner tx, second chance"; return false; }
...
std::unique_ptr<IBlock> core::getBlock(const Crypto::Hash& blockId) {
  ...
  return blockPtr;//return std::move(blockPtr);
}
