...
const command_line::arg_descriptor<std::string> arg_daemon_address = { "daemon-address", "Use daemon instance at <host>:<port> (remote daemon: node1.ultranote.org:33000)", "" };
const command_line::arg_descriptor<std::string> arg_daemon_host = { "daemon-host", "Use daemon instance at host <arg> instead of localhost (remote daemon: node1.ultranote.org)", "" };
...
bool simple_wallet::init(const boost::program_options::variables_map& vm) {
  ...
  std::string walletFileName;
  if (!m_generate_new.empty() || !m_import_new.empty() || !m_track_new.empty() || !m_restore_new.empty()) {
    std::string ignoredString;
